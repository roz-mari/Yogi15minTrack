...
import static org.springframework.test.web.servlet.result.MockMvcResultMatchers.status;
    ...
    class AuthControllerTest {
        ...
        private static String asJson(Object object, ObjectMapper objectMapper) {
            try {
        ...
        void setUp() {
            ...
            class RegisterUserTests {
                ...
                @Test
                @DisplayName("register user -> 200 OK + body")
                void register_ok() throws Exception {
                    ...
                    mockMvc.perform(post("/auth/register")
                            ...
                            .andExpect(content().json(asJson(response, objectMapper)));

                    verify(userService, times(1)).addUser(request);
                }

                @Test
                @DisplayName("register user -> 400 when body invalid")
                void register_bad_request() throws Exception {

                    UserRegisterRequest req = new UserRegisterRequest("john", "john@gmail.com", "");

                    mockMvc.perform(post("/auth/register")
                                    .contentType(MediaType.APPLICATION_JSON)
                                    .content(asJson(req, objectMapper)))
                ...
                class LoginTests {
                    @Test
                    @DisplayName("login -> 200 OK + token")
                    void login_ok() throws Exception {
                        ...
                        mockMvc.perform(post("/auth/login")
                                        ...
                                        .content(asJson(request, objectMapper)))
                                .andExpect(status().isOk())
                                .andExpect(content().json(asJson(jwtResponse, objectMapper)));
                        verify(jwtService, times(1)).loginAuthentication(request);
                    }

                    @Test
                    @DisplayName("login -> 401 when bad credentials")
                ...
                class RegisterAdminTests {
                    ...
                    void admin_registers_admin_ok() throws Exception {
                        UserRegisterRequest request = new UserRegisterRequest("anna", "anna@yogi.com", "Passw0rd!");
                        UserResponse response = new UserResponse(11L, "anna", "anna@yogi.com", "ROLE_ADMIN");
                    ...
                    void user_forbidden_403() throws Exception {
                        UserRegisterRequest request = new UserRegisterRequest("anna", "anna@yogi.com", "Passw0rd!");
                ...
                class MeTests {
                    @Test
                    @DisplayName("me -> 200 OK with profile")
                    void me_ok() throws Exception {
                        UserResponse my = new UserResponse(2L, "maria", "m@e", "ROLE_USER");
                        Mockito.when(userService.getOwnUser(2L)).thenReturn(my);

                        mockMvc.perform(get("/auth/me")
                                        .with(user(userPrincipal)))
                                .andExpect(status().isOk())
                                .andExpect(content().json(asJson(my, objectMapper)));
                    }

                    @Test
                    @DisplayName("me -> 401 when not authenticated")
